...
class TransectHerpetofauna (_database.Base):
    ...
    date_out: Mapped[datetime] = mapped_column(
            DateTime(timezone=True),
            nullable=False
            )
    ...
    rescue_herpetofauna = relationship(
            'RescueHerpetofauna',
            back_populates='transect_herpetofauna'
            )
...
class MarkHerpetofauna (_database.Base):
    ...
    created_at: Mapped[datetime] = mapped_column(
            ...
            nullable=False, default=datetime.now(
                pytz.timezone('America/La_Paz')
                )
            )

    # relationship with rescue_herpetofauna
    rescue_herpetofauna_id: Mapped[int] = mapped_column(
            Integer,
            ForeignKey('rescue_herpetofauna.id')
            )
    rescue_herpetofauna = relationship(
            'RescueHerpetofauna',
            back_populates='mark_herpetofauna'
            )
...
class RescueHerpetofauna (_database.Base):
    ...
    mark_herpetofauna = relationship(
            'MarkHerpetofauna',
            back_populates='rescue_herpetofauna'
            )

    # relationship with transect_herpetofauna
    transect_herpetofauna_id: Mapped[int] = mapped_column(
            Integer,
            ForeignKey('transect_herpetofauna.id')
            )
    transect_herpetofauna = relationship(
...
class TransectHerpetofaunaTranslocation (_database.Base):
    ...
    translocation_herpetofauna = relationship(
            'TranslocationHerpetofauna',
            back_populates='transect_herpetofauna_translocation'
            )
...
class PointHerpetofaunaTranslocation (_database.Base):
    ...
    translocation_herpetofauna = relationship(
            'TranslocationHerpetofauna',
            back_populates='point_herpetofauna_translocation'
            )
...
class TranslocationHerpetofauna (_database.Base):
    ...
    transect_herpetofauna_translocation_id: Mapped[int] = mapped_column(
            Integer,
            ForeignKey('transect_herpetofauna_translocation.id'),
            nullable=True
            )
    transect_herpetofauna_translocation = relationship(
            'TransectHerpetofaunaTranslocation',
            back_populates='translocation_herpetofauna'
            )

    # relationship with point_rescue_herpetofauna_translocation
    point_herpetofauna_translocation_id: Mapped[int] = mapped_column(
            Integer,
            ForeignKey('point_herpetofauna_translocation.id'),
            nullable=True
            )
    point_herpetofauna_translocation = relationship(
    ...
    mark_herpetofauna_id: Mapped[int] = mapped_column(
            Integer,
            ForeignKey('mark_herpetofauna.id'),
            nullable=True
            )
